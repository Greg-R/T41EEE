    ...
    if (updateDisplayFlag == 1) {
      ...
      for (int k = 3; k < 256; k++) {
        if (bands[EEPROMData.currentBand].mode == 0 || bands[EEPROMData.currentBand].mode == DEMOD_AM || bands[EEPROMData.currentBand].mode == DEMOD_SAM) {  //AFP 10-26-22
          ...
          audioYPixel[k] = AUDIOSPECTRUMSCALE + map(15 * log10f((audioSpectBuffer[1024 - k] + audioSpectBuffer[1024 - k + 1] + audioSpectBuffer[1024 - k + 2]) / 3), 0, 100, 0, 120);
          ...
          audioYPixel[k] = AUDIOSPECTRUMSCALE + map(15 * log10f((audioSpectBuffer[k] + audioSpectBuffer[k + 1] + audioSpectBuffer[k + 2]) / 3), 0, 100, 0, 120);
        }
        if (audioYPixel[k] < 0)
          audioYPixel[k] = 0;
      }
